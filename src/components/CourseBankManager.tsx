...
export const CourseBankManager: React.FC<CourseBankManagerProps> = ({
  ...
  // Feedback Notification Banner State for Batch Tagging Actions
  const [batchFeedbackMessage, setBatchFeedbackMessage] = useState<{ text: string; type: 'offered' | 'bank' } | null>(null);

  // Batch Tag Selected as Offered (Active for student registration forms)
  const handleBatchTagAsOffered = () => {
    if (selectedIds.length === 0) return;

    if (activeTab === 'classes') {
      const updated = classList.map((c) =>
        selectedIds.includes(c.id) ? { ...c, isOffered: true } : c
      );
      onUpdateClassList(updated);
    } else {
      const updated = sbaHubOptions.map((s) =>
        selectedIds.includes(s.id) ? { ...s, isOffered: true } : s
      );
      onUpdateSbaHubOptions(updated);
    }

    setBatchFeedbackMessage({
      text: `Successfully tagged ${selectedIds.length} course${selectedIds.length > 1 ? 's' : ''} as Offered (active on student registration form).`,
      type: 'offered',
    });
    setTimeout(() => setBatchFeedbackMessage(null), 5000);
  };

  // Batch Untag Selected from Offerings (Keep in Course Bank Repository only)
  const handleBatchUntagFromOfferings = () => {
    if (selectedIds.length === 0) return;

    if (activeTab === 'classes') {
      const updated = classList.map((c) =>
        selectedIds.includes(c.id) ? { ...c, isOffered: false } : c
      );
      onUpdateClassList(updated);
    } else {
      const updated = sbaHubOptions.map((s) =>
        selectedIds.includes(s.id) ? { ...s, isOffered: false } : s
      );
      onUpdateSbaHubOptions(updated);
    }

    setBatchFeedbackMessage({
      text: `Untagged ${selectedIds.length} course${selectedIds.length > 1 ? 's' : ''} from offerings (stored in Course Bank repository only, removed from registration form).`,
      type: 'bank',
    });
    setTimeout(() => setBatchFeedbackMessage(null), 5000);
  };
  ...
  const offeredCountInCurrentList = currentList.filter((item) => item.isOffered !== false).length;
  const bankedCountInCurrentList = currentList.filter((item) => item.isOffered === false).length;
  ...
  const handleSelectOfferedOnly = () => {
    const offeredIds = currentList.filter((item) => item.isOffered !== false).map((item) => item.id);
    setSelectedIds(offeredIds);
  };

  const handleSelectBankedOnly = () => {
    const bankedIds = currentList.filter((item) => item.isOffered === false).map((item) => item.id);
    setSelectedIds(bankedIds);
  };
  ...
  return (
    <div className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden space-y-6">
      ...
      <div className="p-6 space-y-6">
        ...
        <div className="bg-slate-50 border border-slate-200 p-3.5 rounded-2xl flex flex-col lg:flex-row items-stretch lg:items-center justify-between gap-3 shadow-2xs">
          <div className="flex flex-wrap items-center gap-2">
            <div className="flex items-center gap-2 pr-2 border-r border-slate-300">
              ...
              />
              <label htmlFor="select-all-courses" className="text-xs font-bold text-slate-800 cursor-pointer">
            ...
            <div className="flex items-center gap-1">
              <button
                type="button"
                onClick={handleSelectOfferedOnly}
                ...
                title="Select all courses currently active for registration"
              >
                Offered ({offeredCountInCurrentList})
              </button>
              <button
                type="button"
                onClick={handleSelectBankedOnly}
                className="px-2.5 py-1 text-[11px] font-bold text-slate-700 bg-slate-200/80 hover:bg-slate-300 border border-slate-300 rounded-lg transition-colors cursor-pointer"
                title="Select all courses kept in Course Bank only"
              >
                Bank Only ({bankedCountInCurrentList})
              </button>
            </div>

            {selectedIds.length > 0 && (
              <span className="text-xs font-bold px-2.5 py-0.5 bg-purple-100 text-purple-900 border border-purple-200 rounded-full">
          ...
          {selectedIds.length > 0 && (
            <div className="flex flex-wrap items-center gap-2 w-full lg:w-auto shrink-0 justify-start lg:justify-end">
              {/* PRIMARY BATCH ACTION: Tag as Offered (Make active on registration form) */}
              <button
                type="button"
                onClick={handleBatchTagAsOffered}
                className="px-3 py-1.5 bg-emerald-600 hover:bg-emerald-700 text-white text-xs font-bold rounded-xl transition-all shadow-xs shrink-0 cursor-pointer flex items-center gap-1.5"
                title="Tag selected courses as Offered (make active on student registration form)"
              >
                <CheckCircle className="w-3.5 h-3.5" />
                <span>Tag as Offered ({selectedIds.length})</span>
              </button>

              {/* PRIMARY BATCH ACTION: Untag from Offerings (Keep in Course Bank repository) */}
              <button
                type="button"
                onClick={handleBatchUntagFromOfferings}
                className="px-3 py-1.5 bg-amber-600 hover:bg-amber-700 text-white text-xs font-bold rounded-xl transition-all shadow-xs shrink-0 cursor-pointer flex items-center gap-1.5"
                title="Untag selected courses from offerings (store in Course Bank repository only, removed from registration form)"
              >
                <ToggleLeft className="w-3.5 h-3.5" />
                <span>Untag from Offerings ({selectedIds.length})</span>
              </button>
              ...
              <button
                type="button"
                onClick={() => setIsBatchScheduleModalOpen(true)}
                className="px-3 py-1.5 bg-gradient-to-r from-purple-700 to-indigo-700 hover:from-purple-800 hover:to-indigo-800 text-white text-xs font-bold rounded-xl transition-all shadow-xs shrink-0 cursor-pointer flex items-center gap-1.5"
                title="Batch Schedule selected courses"
              >
              ...
              {activeTab === 'classes' ? (
                <>
                  <button
                    ...
                    className="px-3 py-1.5 bg-gradient-to-r from-purple-600 to-indigo-600 hover:from-purple-700 hover:to-indigo-700 text-white text-xs font-bold rounded-xl transition-all shadow-xs shrink-0 cursor-pointer flex items-center gap-1.5"
                    title="Move selected classes to SBA Hub"
                  >
                    <RefreshCw className="w-3.5 h-3.5" />
                    <span>Convert to SBA</span>
                  </button>
                  <button
                    type="button"
                    onClick={handleDuplicateClassesToSba}
                    className="px-3 py-1.5 bg-white border border-purple-300 text-purple-700 hover:bg-purple-50 text-xs font-bold rounded-xl transition-all shadow-xs shrink-0 cursor-pointer flex items-center gap-1.5"
                    title="Duplicate selected classes to SBA Hub"
                  >
                    <Copy className="w-3.5 h-3.5" />
                    <span>Duplicate to SBA</span>
                  </button>
                  <button
                    type="button"
                    onClick={() => {
                    ...
                    className="px-3 py-1.5 bg-white border border-red-300 text-red-600 hover:bg-red-50 text-xs font-bold rounded-xl transition-all shadow-xs shrink-0 cursor-pointer flex items-center gap-1.5"
                ...
                <>
                  <button
                    ...
                    className="px-3 py-1.5 bg-gradient-to-r from-emerald-600 to-teal-600 hover:from-emerald-700 hover:to-teal-700 text-white text-xs font-bold rounded-xl transition-all shadow-xs shrink-0 cursor-pointer flex items-center gap-1.5"
                  ...
                  <button
                    ...
                    className="px-3 py-1.5 bg-white border border-emerald-300 text-emerald-700 hover:bg-emerald-50 text-xs font-bold rounded-xl transition-all shadow-xs shrink-0 cursor-pointer flex items-center gap-1.5"
                    title="Duplicate selected SBA Hub options to Regular Classes"
                  >
                    <Copy className="w-3.5 h-3.5" />
                    <span>Duplicate to Regular</span>
                  </button>
                  <button
                    type="button"
                    onClick={() => {
              ...
              <button
                ...
                className="px-2.5 py-1.5 text-xs font-semibold text-slate-500 hover:text-slate-800 hover:bg-slate-200 rounded-lg transition-colors cursor-pointer"
              >
                Clear
              </button>
            </div>
        ...
        {/* Batch Feedback Notification Banner */}
        {batchFeedbackMessage && (
          <div
            className={`p-3.5 rounded-2xl border flex items-center justify-between gap-3 animate-fade-in ${
              batchFeedbackMessage.type === 'offered'
          ...
          >
            <div className="flex items-center gap-2 text-xs font-bold">
              ...
              <span>{batchFeedbackMessage.text}</span>
            </div>
            <button
              type="button"
              onClick={() => setBatchFeedbackMessage(null)}
                ...
                return (
                  ...
                  <div className="pt-4 mt-3 border-t border-slate-100 flex items-center justify-between gap-2">
                    <button
                      ...
                      title={isOfferedNow ? 'Untag and keep stored in Course Bank only (removes from registration form)' : 'Tag as Offered and make available on public registration form'}
                    >
                      {isOfferedNow ? <ToggleRight className="w-4 h-4 text-amber-600" /> : <ToggleLeft className="w-4 h-4" />}
                      <span>{isOfferedNow ? 'Untag from Offerings' : 'Tag as Offered'}</span>
                    </button>
