...
import { Department, SchoolUser, LocationOption, isDepartmentVisibleToStudents } from '../../types';
...
export const AdminDepartmentManagement: React.FC<AdminDepartmentManagementProps> = ({
  ...
  const [activeSubTab, setActiveSubTab] = useState<'departments' | 'visibility' | 'locations'>('departments');
  ...
  const [showToStudents, setShowToStudents] = useState<boolean>(true);
  ...
  const openEditModal = (dept: Department) => {
    ...
    setShowToStudents(dept.showToStudents !== undefined ? dept.showToStudents : isDepartmentVisibleToStudents(dept));
    setIsAddingInlineLocation(false);
    setIsModalOpen(true);
  };
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap items-center gap-2 border-b border-slate-200 pb-2">
        ...
        >
          ...
          <span>Student Filters Visibility ({departments.filter(d => isDepartmentVisibleToStudents(d)).length}/{departments.length} Shown)</span>
        </button>
                  ...
                  <div className="p-5 space-y-4">
                    ...
                    <div className="flex items-center justify-between gap-2 pt-2 border-t border-slate-100">
                      ...
                      <button
                        ...
                        disabled={isReadOnly}
                        onClick={() => {
                          const currentVis = isDepartmentVisibleToStudents(dept);
                          onUpdateDepartment({
                            ...dept,
                            showToStudents: !currentVis,
                          });
                        }}
                        className={`px-2.5 py-1 text-[11px] font-bold rounded-lg border flex items-center gap-1.5 transition-all cursor-pointer ${
                          isDepartmentVisibleToStudents(dept)
                            ? 'bg-emerald-50 text-emerald-700 border-emerald-200 hover:bg-emerald-100'
                            : 'bg-slate-100 text-slate-600 border-slate-200 hover:bg-slate-200'
                        }`}
                        title="Toggle whether students can see this department in class selection filters & chipbars"
                      >
                        {isDepartmentVisibleToStudents(dept) ? (
                          <>
      ...
      {/* TAB 2: STUDENT FILTERS & CHIPBARS VISIBILITY SETTINGS */}
      {activeSubTab === 'visibility' && (
        <div className="space-y-6">
          <div className="bg-gradient-to-r from-purple-900 via-indigo-900 to-slate-900 text-white p-6 rounded-3xl shadow-lg border border-purple-800">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
              <div>
                <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-purple-800/80 text-purple-200 text-xs font-bold mb-2">
                  <Sliders className="w-3.5 h-3.5 text-emerald-400" />
                  <span>Student Registration & Catalog Filters Control</span>
                </div>
                <h3 className="text-xl font-bold">Student Department Selection Settings</h3>
                <p className="text-xs text-purple-200 mt-1 max-w-2xl leading-relaxed">
                  Select which academic departments appear in student class registration chipbars, SBA Hub department options, and catalog filter dropdowns. Administrative and registrar departments are hidden by default to keep student forms focused strictly on course selection.
                </p>
              </div>
              ...
              {!isReadOnly && (
                <div className="flex items-center gap-2 shrink-0">
                  <button
                    ...
                    onClick={() => {
                      departments.forEach((d) => {
                        const isNavAdmin = d.name.toLowerCase().includes('admin') || d.name.toLowerCase().includes('registrar');
                        onUpdateDepartment({ ...d, showToStudents: !isNavAdmin });
                      });
                    }}
                  ...
                  >
                    ...
                    <span>Reset Defaults (Hide Admin)</span>
                  </button>
                </div>
              )}
            </div>
          ...
          {/* Departments Visibility Toggle Table */}
          <div className="bg-white rounded-3xl border border-slate-200 shadow-xs overflow-hidden">
            <div className="p-4 bg-slate-50 border-b border-slate-200 flex items-center justify-between font-bold text-xs text-slate-600">
              ...
              <span>Student Filter Visibility Toggle</span>
            </div>

            <div className="divide-y divide-slate-100">
              {departments.map((dept) => {
                const isVisible = isDepartmentVisibleToStudents(dept);
                const isAdminDept = dept.name.toLowerCase().includes('admin') || dept.name.toLowerCase().includes('registrar');

                return (
                  <div key={dept.id} className="p-4 sm:p-5 flex flex-col sm:flex-row sm:items-center justify-between gap-4 hover:bg-slate-50/60 transition-colors">
                    <div className="flex items-start gap-3">
                      <div className={`w-10 h-10 rounded-xl ${dept.color} text-white flex items-center justify-center font-bold text-xs shrink-0 mt-0.5 shadow-2xs`}>
                        {dept.code}
                      </div>
                      <div>
                        <div className="flex items-center gap-2">
                        ...
                        <p className="text-xs text-slate-500 mt-0.5 max-w-lg">{dept.description || 'No description available.'}</p>
                        <div className="flex items-center gap-3 mt-2 text-[11px] text-slate-400">
                          <span>Room: {dept.room || 'Unassigned'}</span>
                          <span>•</span>
                          <span>Staff: {users.filter(u => u.departmentId === dept.id).length} assigned</span>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="flex items-center gap-3 self-end sm:self-center shrink-0">
                      <div className="text-right hidden md:block">
                        <span className={`block text-xs font-bold ${isVisible ? 'text-emerald-700' : 'text-slate-500'}`}>
                          {isVisible ? 'Shown to Students' : 'Hidden from Students'}
                        </span>
                        <span className="text-[10px] text-slate-400">
                          {isVisible ? 'Appears in student registration & filters' : 'Excluded from student selection fields'}
                        </span>
                      </div>
                      ...
                      <button
                        ...
                        onClick={() => {
                          onUpdateDepartment({
                            ...dept,
                            showToStudents: !isVisible,
                          });
                        }}
                        className={`relative inline-flex h-7 w-14 shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none ${
                          isVisible ? 'bg-purple-600' : 'bg-slate-300'
                        } ${isReadOnly ? 'opacity-50 cursor-not-allowed' : ''}`}
                        title={isVisible ? 'Click to hide from student filters' : 'Click to show in student filters'}
                      >
                        <span
              ...
              <div className="flex-1 overflow-y-auto p-6 space-y-4">
                ...
                <div className="p-3.5 bg-slate-50 border border-slate-200 rounded-2xl space-y-1">
                  <label className="flex items-center gap-2.5 cursor-pointer font-bold text-xs text-slate-900">
                    ...
                    />
                    <span>Show in Student Class Registration & Chipbar Filters</span>
                  </label>
                  <p className="text-[11px] text-slate-500 pl-6 leading-normal">
                    When checked, students can filter and register for classes under this department. Uncheck to hide administrative or registrar departments from student class views.
                  </p>
                </div>
              </div>
