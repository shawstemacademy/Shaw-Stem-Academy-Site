...
export const LoginPage: React.FC<LoginPageProps> = ({
    ...
    try {
      ...
      if (result) {
        ...
        await logSecurityEvent({
          ...
          status: 'success',
          email: result.user.email || 'google_user',
      ...
      if (err?.code === 'auth/unauthorized-domain') {
        ...
        errMsg = `Domain Unauthorized: The domain "${domain}" is not authorized in your Firebase Console. Please add it to Authentication > Settings > Authorized Domains.`;
      } else if (err?.code === 'auth/popup-blocked') {
        errMsg = 'Pop-up Blocked: Please enable pop-ups for this site, or open this app in a New Tab to sign in with Google.';
      } else if (err?.code === 'auth/operation-not-supported-in-this-environment') {
        errMsg = 'Environment Error: Pop-ups are not supported inside this iframe. Please open this app in a New Tab to sign in.';
      } else {
        errMsg = `Google Sign-In Error: ${errMsg}`;
      }
      setLoginMessage(errMsg);
      await logSecurityEvent({
        eventType: 'failed_login',
        status: 'failure',
        details: `Google login failed: ${errMsg}`,
      });
    } finally {
  ...
  const handleManualLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Evaluate reCAPTCHA Enterprise token if generated by widget
      const recaptchaToken = getRecaptchaToken();
      if (recaptchaToken) {
        try {
          const assessment = await createRecaptchaAssessment(recaptchaToken, 'LOGIN');
          ...
          console.warn('reCAPTCHA Assessment note:', recaptchaErr);
        }
      }
      ...
      if (matchedUser) {
        ...
        await logSecurityEvent({
          ...
          details: `Logged in successfully as ${matchedUser.role}`,
        });
        ...
        await logSecurityEvent({
          ...
          status: 'failure',
          email: cleanEmail,
          details: 'No user profile found for email',
        });
        return;
      }
    } finally {
          ...
          <>
            ...
            <form onSubmit={handleManualLogin} className="space-y-4">
              ...
              <button
                type="submit"
                disabled={authLoading}
                className="w-full py-3 bg-slate-900 dark:bg-blue-600 hover:bg-slate-800 dark:hover:bg-blue-500 text-white font-bold text-xs rounded-xl shadow-md transition-colors flex items-center justify-center gap-2 mt-2 cursor-pointer disabled:opacity-50"
              >
                {authLoading ? (
